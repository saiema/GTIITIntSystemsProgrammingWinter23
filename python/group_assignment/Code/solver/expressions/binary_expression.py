...
class BinaryExpression(Expression):
    ...
    def variables(self: 'BinaryExpression') -> List[str]:
        vars_set: List[str] = []
    ...
    def interpretations(self: 'BinaryExpression') -> List[Dict[str, bool]]:
        interpretations_lists: List[List[Tuple(str, bool)]] = Expression._recursive_interpretations([[]], self.variables())
    ...
    def evaluate(self: 'BinaryExpression', interpretation: Dict[str, bool]) -> bool:
        ...
        elif self.__type == BinaryOp.OR:
            return self.__left.evaluate(interpretation) or self.__right.evaluate(interpretation)
        elif self.__type == BinaryOp.IMPLIES:
            return (not self.__left.evaluate(interpretation)) or self.__right.evaluate(interpretation)
        elif self.__type == BinaryOp.IFF:
            return self.__left.evaluate(interpretation) == self.__right.evaluate(interpretation)
        elif self.__type == BinaryOp.XOR:
            return self.__left.evaluate(interpretation) != self.__right.evaluate(interpretation)
